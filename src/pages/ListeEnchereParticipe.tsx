...
export const ListeEnchereParticipe: React.FC = () => {
  ...
  const [message, setMessage ] = useState('Attente de résultat...');
  ...
  useEffect(() =>{
    ...
    fetch('https://encherews-production.up.railway.app/Enchere/encheres/historique/utilisateur/' + iduser, {
        method: "GET",
      })
       .then(function(response) {
        if(response.ok) {
          ...
          jsonPromise.then(data => {
            ...
            setPosts(data);       
            setMessage("");
            if (posts.length == 0) setMessage2("Vous n'avez pas encore participé à une enchère.");
            return jsonPromise;
          }).catch(error => {
