...
const Insertion_enchere: React.FC = () => {
  ...
  async function handleSubmit() {
       ...
       .then(function(response) {
        if(response.ok) {
          ...
          jsonPromise.then(data => {
            ...
            addPhotos();            
            setMessage("Insérée avec succès!");
            return jsonPromise;
          }).catch(error => {
            setMessage("Insérée avec succès!");
            console.log("Successful request, Could not parse body as json", error);
    ...
    function addPhotos() {
      ...
      fetch('https://encherews-production.up.railway.app/Enchere/encheres/photos', {
          method: "POST",
  ...
  return (
    <IonPage>
        ...
        <IonList>
          ...
          <IonButton onClick={handleSubmit} expand='block' shape='round' className='ion-margin-top' color="tertiary">
              Créer
          </IonButton>
